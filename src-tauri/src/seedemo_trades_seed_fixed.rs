...
pub async fn seed_demo_forex_trades(db: &Surreal<Db>) -> Result<(), String> {
    ...
    for i in 1..=60 {
        ...
        let trade = Trade {
            ...
            account_id: Some("account:demo_forex".to_string()),
            result,
            quantity,
            ...
            images: vec![],
            partial_exits: crate::models::SurrealJson(serde_json::json!([])),
            asset_id: None,
...
pub async fn seed_demo_b3_acoes_trades(db: &Surreal<Db>) -> Result<(), String> {
    ...
    for i in 1..=55 {
        ...
        let trade = Trade {
            ...
            asset_symbol: asset.to_string(),
            strategy_id: Some(strategy.to_string()),
            account_id: Some("account:demo_b3_acoes".to_string()),
            result,
            quantity,
            direction: if is_long { "Buy".to_string() } else { "Sell".to_string() },
            entry_price,
...
pub async fn seed_demo_b3_futuros_trades(db: &Surreal<Db>) -> Result<(), String> {
    ...
    for i in 1..=70 {
        ...
        let trade = Trade {
            ...
            asset_symbol: asset.to_string(),
            strategy_id: Some(strategy.to_string()),
            account_id: Some("account:demo_b3_futuros".to_string()),
            result,
            quantity,
            direction: if is_long { "Buy".to_string() } else { "Sell".to_string() },
            entry_price,
...
pub async fn seed_demo_nasdaq_trades(db: &Surreal<Db>) -> Result<(), String> {
    ...
    for i in 1..=60 {
        ...
        let trade = Trade {
            ...
            asset_symbol: asset.to_string(),
            strategy_id: Some(strategy.to_string()),
            account_id: Some("account:demo_nasdaq".to_string()),
            result,
            quantity,
            direction: if is_long { "Buy".to_string() } else { "Sell".to_string() },
            entry_price,
...
pub async fn seed_demo_crypto_trades(db: &Surreal<Db>) -> Result<(), String> {
    ...
    for i in 1..=65 {
        ...
        let trade = Trade {
            ...
            asset_symbol: asset.to_string(),
            strategy_id: Some(strategy.to_string()),
            account_id: Some("account:demo_crypto".to_string()),
            result,
            quantity,
            direction: if is_long { "Buy".to_string() } else { "Sell".to_string() },
            entry_price,

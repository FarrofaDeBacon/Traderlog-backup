...
pub async fn seed_strategies(db: &Surreal<Db>, filter: Option<Vec<String>>) -> Result<(), String> {
    let mut result = db
        .query("SELECT *, meta::id(id) as id FROM strategy LIMIT 1")
        .await
    ...
    let strategies = vec![
        (
            "s1",
            "Barra Elefante",
            "Candle com corpo muito maior que os anteriores",
            "Volume acima da média",
        ),
        (
            "s2",
            "Bear 180",
            "Reversão baixista forte",
            "Stop acima da máxima",
        ),
        (
            "s3",
            "Breakout",
            "Rompimento de resistência/suporte",
            "Volume confirmatório",
        ),
        (
            "s4",
            "Bull 180",
            "Reversão altista forte",
            "Stop abaixo da mínima",
        ),
        (
            "s5",
            "Clássico Perto da Média",
            "Pullback em tendência",
            "Retorno à média móvel",
        ),
        (
            "s6",
            "Gift",
            "Pequeno candle após Barra Elefante",
    ...
    for (id, name, description, entry) in strategies {
        ...
        db.query("UPSERT type::thing('strategy', $id) CONTENT $data")
            .bind(("id", id))
            .bind(("data", json_data))
            .await
            .map_err(|e| e.to_string())?;

...
use crate::models::{RiskProfile, GrowthPhase, GrowthPhaseProgressionRule, GrowthPhaseRegressionRule};
...
pub async fn seed_risk_profiles(db: &Surreal<Db>, filter: Option<Vec<String>>) -> Result<(), String> {
    ...
    let profiles = vec![
        ("r1", "Conservador", 50.0, 100.0, 0.5, 3, 2.0, true, "All", true),
        ("r2", "Moderado", 150.0, 300.0, 1.0, 5, 1.5, true, "All", true),
        ("r3", "Agressivo", 500.0, 1000.0, 2.0, 10, 1.0, false, "All", true),
    ];
    ...
    for (id, name, max_loss, target, risk_per_trade, max_trades, min_rr, lock, account_type, growth_enabled) in profiles {
            ...
            vec![
                GrowthPhase {
                    ...
                    progression_rules: vec![
                        GrowthPhaseProgressionRule { condition: "profit_target".to_string(), value: target * 5.0 },
                        GrowthPhaseProgressionRule { condition: "consistency_days".to_string(), value: 20.0 },
                    ],
                ...
                GrowthPhase {
                    ...
                    progression_rules: vec![
                        GrowthPhaseProgressionRule { condition: "profit_target".to_string(), value: target * 15.0 },
                    ],
                    regression_rules: vec![],
                },
            ]
        ...
        let data = RiskProfile {
            ...
            current_phase_index: 0, growth_phases,
